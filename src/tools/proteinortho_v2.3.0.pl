...
#  print STDERR "blastall -d $db -i $query -p $blast -a $cpu -m8 -e $evalue >  $out\n";
  ...
  #	`/nethome/cisseoh/utils/oldBlastall/blast-2.2.11/bin/blastall -d $db -i $query -p $blast -a 1 -m8 -e $evalue >  $out`;
}
...
#------------------------------------------------------------------------------
  if ($blast =~ m/^blastp$/i) {
    ...
    # `/nethome/cisseoh/utils/oldBlastall/blast-2.2.11/bin/formatdb -i $faa -p T -o F`;   
  #  print STDERR "done\n";
  }
  elsif ($blast =~ m/^blastn$/i) {
    print STDERR "Formating database $faa...\n";
    `formatdb -i $faa -p F -o F`;
    # `/nethome/cisseoh/utils/oldBlastall/blast-2.2.11/bin/formatdb -i $faa -p F -o F`;	
   # print STDERR "done.\n";
  }
  else {
    die "$blast is not supported yet! Feel free to adapt the code to your needs. The programmer.\n"
